...
<x-app-layout>
    ...
    <div class="py-6 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            {{-- Filter tabs --}}
            <div class="flex gap-1 bg-white border border-gray-100 p-1 flex-1">
                @foreach([
                ...
                <a href="{{ route('notifications.index', ['filter' => $key]) }}"
                   class="flex-1 text-center px-3 py-1.5 text-xs font-medium transition flex items-center justify-center gap-1.5
                          {{ $filter === $key
                              ? 'bg-blue-600 text-white'
                              : 'text-gray-500 hover:bg-gray-50' }}">
                    {{ $tab['label'] }}
                    @if($tab['count'] > 0)
                    <span class=" px-1.5 py-0.5 text-[10px] font-bold leading-none
            ...
            <div class="flex gap-2 flex-shrink-0">
                ...
                <button @click="selectMode = !selectMode; if(!selectMode) selected = []"
                        ...
                        class="px-3 py-1.5 border text-xs font-medium hover:border-blue-300 transition flex items-center gap-1.5">
                ...
                <form method="POST" action="{{ route('notifications.read-all') }}">
                    @csrf
                    <button class="px-3 py-1.5 border border-gray-200 bg-white text-xs font-medium
        ...
        <div x-show="selectMode"
             ...
             class="bg-blue-50 border border-blue-100 px-4 py-3 flex items-center gap-3"
             style="display:none">
        ...
        <div class="group relative bg-white border transition
            ...
            <div x-show="selectMode"
                 ...
                 class="absolute inset-0 z-10 cursor-pointer flex items-start justify-end p-3"
                 style="display:none">
                <div :class="isSelected('{{ $n->_id }}')
                        ? 'bg-blue-600 border-blue-600'
                        : 'bg-white border-gray-300 group-hover:border-blue-400'"
                     class="w-5 h-5 border-2 flex items-center justify-center transition flex-shrink-0">
