...
{{-- 
══════════════════════════════════════════════════════════════════
  CARA KONEKSI API LOKASI KENDARAAN (REAL-TIME)
══════════════════════════════════════════════════════════════════

OPSI 1 — DATABASE POLLING (Termudah, <5 menit setup)
────────────────────────────────────────────────────
Simpan koordinat di tabel/collection vehicles:
  vehicles: { lat: float, lon: float, updated_at: timestamp }

Di controller (DashboardController.php):
  $vehicleLocations = Vehicle::select('_id','plate_number','driver_name','status','lat','lon')->get();
  return view('admin.dashboard', compact('vehicleLocations', ...));

Di blade, ganti data-lat/data-lon pada tiap .vehicle-pin dengan:
  data-lat="{{ $vehicle->lat }}"
  data-lon="{{ $vehicle->lon }}"

Konversi koordinat real ke posisi SVG map (viewBox 900×420):
  x = (lon - 95) / (141 - 95) * 820 + 60
  y = ((-lat) - (-6)) / (17) * 240 + 60

Contoh di blade:
  @foreach($vehicleLocations as $v)
  @php
    $x = ($v->lon - 95) / 46 * 820 + 60;
    $y = ((-$v->lat) + 6) / 17 * 240 + 60;
  @endphp
  <g class="vehicle-pin" data-id="{{ $v->_id }}" ...
...
──────────────────────────────────────────────────────
OPSI 2 — WEBSOCKET / PUSHER (Real-time tanpa refresh)
──────────────────────────────────────────────────────
Install: composer require pusher/pusher-php-server
         npm install laravel-echo pusher-js

1. Buat Event: php artisan make:event VehicleLocationUpdated
   → implements ShouldBroadcast
   → broadcastOn(): return new Channel('vehicles');
   → public $vehicleId, $lat, $lon, $status;

2. Dari device/driver app, hit endpoint:
   POST /api/vehicles/{id}/location
   body: { lat, lon }

3. Di controller: event(new VehicleLocationUpdated($id, $lat, $lon));

4. Di blade JS:
   Echo.channel('vehicles').listen('VehicleLocationUpdated', (data) => {
       updatePinPosition(data.vehicleId, data.lat, data.lon);
   });

   function updatePinPosition(id, lat, lon) {
       const x = (lon - 95) / 46 * 820 + 60;
       const y = ((-lat) + 6) / 17 * 240 + 60;
       const pin = document.querySelector(`[data-id="${id}"]`);
       if (pin) pin.setAttribute('transform', `translate(${x},${y})`);
   }

──────────────────────────────────────────────────────
OPSI 3 — GPS TRACKER (Hardware seperti Teltonika/Concox)
──────────────────────────────────────────────────────
Tracker mengirim koordinat ke server via HTTP/MQTT.
Simpan ke DB, lalu gunakan Opsi 1 atau 2 di atas.

Provider populer di Indonesia:
  - Teltonika (FMB920)
  - Concox (GT06)
  - Startrack GPS
Biasanya pakai protokol TCP yang perlu socket server,
atau ada platform SaaS (Wialon, Traccar) yang expose REST API.
══════════════════════════════════════════════════════════════════
--}}