...
<x-app-layout>
    ...
    <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
        ...
        {{-- Filter tabs --}}
        <div class="flex gap-1 bg-white border border-gray-100 p-1 overflow-x-auto">
            @foreach([
                'all'         => 'Semua',
                'open'        => 'Terbuka',
            ...
            <a href="{{ route('admin.tickets.index', ['status' => $key]) }}"
               class="flex-shrink-0 px-3 py-1.5 text-xs font-medium transition flex items-center gap-1.5
                      {{ $status === $key
               ...
               class="flex items-start gap-4 px-5 py-4 border-b border-gray-50 hover:bg-gray-50
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-0.5">
                        ...
                        @if($ticket->priority === 'urgent')
                        <span class="px-2 py-0.5 text-[10px] font-bold bg-red-100 text-red-600 flex-shrink-0">URGENT</span>
                        @endif
                    </div>
