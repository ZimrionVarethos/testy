        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-2">
                ...
                <div class="text-sm space-y-1.5">
                    ...
                    <div class="flex justify-between"><span class="text-gray-500">Telepon</span>
                        <a href="tel:{{ $booking->user['phone'] ?? '' }}" class="text-indigo-500">{{ $booking->user['phone'] ?? $booking->user['email'] }}</a>
                    </div>
                </div>
                ...
                <div class="text-sm space-y-1.5">
                    ...
                    <div class="flex justify-between"><span class="text-gray-500">Plat</span><span class="font-medium">{{ $booking->vehicle['plate_number'] }}</span></div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 col-span-full space-y-2">
                ...
                <div class="text-sm space-y-2">
                    <div class="flex items-start gap-2">
                        ...
                        <div><p class="text-gray-500 text-xs">Penjemputan</p><p>{{ $booking->pickup['address'] }}</p></div>
                    </div>
                    @if($booking->dropoff)
                    <div class="flex items-start gap-2">
                        <span class="h-5 w-5 rounded-full bg-red-100 text-red-600 text-xs flex items-center justify-center shrink-0 mt-0.5">B</span>
                        <div><p class="text-gray-500 text-xs">Tujuan</p><p>{{ $booking->dropoff['address'] }}</p></div>
                    </div>
                    @endif
                    @if($booking->notes)
