...
<x-app-layout>
    ...
    <div class="py-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-3">
        @if($errors->any())<div class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 text-sm">{{ $errors->first('error') }}</div>@endif
        @if($hasActiveBooking)
            {{-- Banner warning di atas list --}}
            <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 text-sm mb-4">
            ...
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span class="px-2 py-0.5 bg-yellow-100 text-yellow-700 text-xs font-medium">Menunggu Driver</span>
                    </div>
                    ...
                    <p class="font-semibold text-blue-600 mt-1">Rp {{ number_format($b->total_price, 0, ',', '.') }}</p>
                </div>
                @if($hasActiveBooking)
                    <button disabled class="px-4 py-2 bg-gray-200 text-gray-400 text-sm font-medium whitespace-nowrap cursor-not-allowed">
                        Tidak Tersedia
                    </button>
                @else
                    <form method="POST" action="{{ route('driver.bookings.accept', $b->_id) }}" class="ml-4">
                        @csrf
                        <button class="px-4 py-2 bg-blue-600 text-white text-sm hover:bg-blue-700 transition font-medium whitespace-nowrap">
                            Ambil Pesanan
                        </button>
                    </form>
                @endif
