    ...
    <div class="py-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
                ...
                @else
                <p class="text-sm text-gray-400">Belum ada driver.</p>
                @endif
            </div>
        ...
        <div class="flex gap-3">
            <form method="POST" action="{{ route('admin.bookings.confirm', $booking->_id) }}">
                @csrf
                <button class="px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition">Konfirmasi Pesanan</button>
            </form>
            <form method="POST" action="{{ route('admin.bookings.cancel', $booking->_id) }}" onsubmit="return confirm('Batalkan pesanan?')">
                @csrf
                <button class="px-4 py-2 bg-red-500 text-white text-sm rounded-lg hover:bg-red-600 transition">Batalkan</button>
            </form>
        </div>
        @endif
    </div>
